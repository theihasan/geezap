        ...
        <div class="max-w-7xl mx-auto px-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div class="col-span-full md:col-span-3">
                    @forelse($jobs as $job)
                        <div
                            class="bg-[#1a1a3a] rounded-2xl border border-gray-700 p-6 hover:border-pink-500/50 transition mb-6 font-ubuntu">
                            <div class="flex justify-between items-start">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-gray-300">{{ $job->employer_name }} •
                            ...
                            <div class="text-gray-400 text-sm space-y-1 mt-4 font-ubuntu">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span>Posted: <span
                                            class="text-white">{{ $job->posted_at?->diffForHumans() }}</span></span>
                                </div>
                                @if ($job->state && $job->country)
                                    <div class="flex items-center gap-2">
                                        <i class="las la-map-marker-alt text-pink-300"></i>
                                        <span>Location: <span class="text-white">{{ $job->state }},
                            ...
                            <div class="flex flex-wrap gap-2 mt-4">
                                <span
                                    class="px-3 py-1 bg-pink-500/10 text-pink-300 rounded-full text-sm font-oxanium-semibold">
                                    {{ $job->category->name }}
                                </span>
                                <span
                                    class="px-3 py-1 bg-pink-500/10 text-pink-300 rounded-full text-sm font-oxanium-semibold">
                    ...
                    <!-- Pagination -->
                    {{ $jobs->links('v2.partials.pagination') }}
                </div>
            </div>
        </div>
    </section>
