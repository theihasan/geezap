...
@section('content')
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <!-- Preferences Form -->
            <div>
                <form action="{{ route('guest.preferences.update') }}" method="POST" class="space-y-6">
                    @csrf
                            ...
                            @foreach($jobCategories->take(12) as $category)
                                <label class="relative cursor-pointer">
                                    <input type="checkbox" name="preferred_job_topics[]" value="{{ $category->id }}" 
                                           ...
                                           class="sr-only peer">
                                    <div class="p-2 bg-white/5 hover:bg-white/10 border border-gray-700 peer-checked:border-pink-500 peer-checked:bg-pink-500/10 rounded-lg transition-all text-center">
                                        <div class="text-white font-ubuntu-medium text-xs">{{ $category->name }}</div>
                                    </div>
                                </label>
                            @endforeach
                        </div>
                            ...
                            @foreach($countries->take(20) as $country)
                                <label class="relative cursor-pointer">
                                    <input type="checkbox" name="preferred_regions[]" value="{{ $country->id }}" 
                                           ...
                                           class="sr-only peer">
                                    <div class="p-2 bg-white/5 hover:bg-white/10 border border-gray-700 peer-checked:border-pink-500 peer-checked:bg-pink-500/10 rounded-lg transition-all text-center">
                                        <div class="text-white font-ubuntu-medium text-xs">{{ $country->name }}</div>
                                    </div>
                                </label>
                            @endforeach
                        </div>
                                ...
                                @foreach(['full-time' => 'Full Time', 'part-time' => 'Part Time', 'contract' => 'Contract', 'freelance' => 'Freelance'] as $value => $label)
                                    <label class="relative cursor-pointer">
                                        <input type="checkbox" name="preferred_job_types[]" value="{{ $value }}" 
                                               ...
                                               class="sr-only peer">
                                        <div class="p-3 bg-white/5 hover:bg-white/10 border border-gray-700 peer-checked:border-pink-500 peer-checked:bg-pink-500/10 rounded-lg transition-all text-center">
                    ...
                    <!-- Save Button -->
                    <button type="submit" 
                            class="w-full bg-gradient-to-r from-pink-500 to-purple-600 hover:opacity-90 text-white px-6 py-3 rounded-xl transition-all flex items-center justify-center gap-2 font-oxanium-semibold">
