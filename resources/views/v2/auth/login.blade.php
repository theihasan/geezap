...
@section('content')
    ...
    <div class="bg-white dark:bg-[#12122b] rounded-2xl shadow-2xl p-12 max-w-md w-full space-y-8 border border-gray-200 dark:border-gray-700">
        <div class="text-center space-y-4">
            ...
            <h2 class="text-3xl font-bold text-gray-900 dark:text-white font-oxanium-bold">Welcome to Geezap</h2>
            <p class="text-gray-600 dark:text-gray-400 font-sans">Continue with your preferred social platform</p>
        </div>

        @session('status')
        <div class="flex justify-center">
            <span class="text-red-500 text-center bg-red-50 dark:bg-red-900/20 px-4 py-2 rounded-lg">{{$value}}</span>
        </div>
        @endsession
        ...
        <!-- Login Form -->
        <form action="{{ route('login') }}" method="POST" class="space-y-6 font-sans" id="loginForm">
            @csrf
        ...
        <p class="text-center text-gray-600 dark:text-gray-400 mt-8 font-sans text-sm">
            New to Geezap?
            <a href="{{ route('register') }}" class="text-blue-600 dark:text-purple-400 hover:text-blue-700 dark:hover:text-purple-300 transition-colors font-medium ml-1">
                Create an account
            </a>
        </p>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (form && csrfTokenInput) {
                form.addEventListener('submit', function(e) {
                    ...
                    .catch(error => {
                        console.log('CSRF token refresh failed, continuing with existing token');
                    });
                });

                // Auto-refresh CSRF token every 30 seconds to prevent expiration
                setInterval(function() {
                    ...
                    .catch(error => {
                        console.log('Auto CSRF refresh failed');
                    });
                }, 30000); // 30 seconds
            }
        });
