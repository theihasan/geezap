        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
            <div class="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6">
                ...
                <div class="flex items-center gap-3 mt-4 sm:mt-0">
                    <button
                        class="bg-white/5 hover:bg-white/10 text-white px-4 py-2 rounded-xl transition-colors flex items-center gap-2 font-ubuntu-medium text-sm sm:text-base">
                    ...
                    <div class="space-y-4 font-ubuntu-light">
                        ...
                        @if(auth()->user()->dob)
                            <div>
                                ...
                                <div
                                    class="text-white font-ubuntu-regular">{{ Carbon\Carbon::parse(auth()->user()->dob)->format('F d, Y') }}</div>
                            </div>
                        @endif
                        <div>
                        ...
                        @if(auth()->user()->skills)
                            ...
                            @if(isset($skillsData['skill']))
                                @foreach($skillsData['skill'] as $index => $skill)
                                    @php
                                        $skillLevel = $skillsData['skill_level'][$index] ?? 'beginner';
                                        $skillLevel = strtolower($skillLevel);
                                        $percentage = match ($skillLevel) {
                                            SkillProficiency::BEGINNER->value => 'bg-pink-500/30',
                                            SkillProficiency::INTERMEDIATE->value => 'bg-pink-500/50',
                                            SkillProficiency::PROFICIENT->value => 'bg-pink-500/70',
                            ...
                            @if(auth()->user()->experience)
                                ...
                                @if(isset($experienceData['position']))
                                    @foreach($experienceData['position'] as $index => $position)
                                        ...
                                        <div
                                            class="border-l-2 border-pink-500/20 pl-4 sm:pl-6 {{ !$loop->last ? 'pb-6' : '' }} relative">
                                            <div
                                                class="absolute -left-[9px] top-0 w-4 h-4 rounded-full {{ $loop->first ? 'bg-pink-500' : 'bg-pink-500/50' }}"></div>
                                            <div
                                                ...
                                                <div>
                                                    ...
                                                    <p class="text-pink-400">{{ $experienceData['company_name'][$index] ?? '' }}</p>
                                                </div>
                                    ...
                                    @endforeach
                                @else
                                    <p class="text-gray-400">No experience added yet.</p>
                                @endif
                            @else
                                <p class="text-gray-400">No experience added yet.</p>
                            @endif
