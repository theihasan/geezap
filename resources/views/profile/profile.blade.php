                    ...
                    <div class="md:w-full">
                        <div class="relative flex items-end justify-between">
                            <div class="relative flex items-end">
                                <img src="{{asset('assets/images/profile.jpg')}}" class="size-28 rounded-full shadow dark:shadow-gray-800 ring-4 ring-slate-50 dark:ring-slate-800" alt="">
                                <div class="ms-4">
                            ...
                            @if(auth()->user()->skills)
                                @foreach(json_decode(auth()->user()->skills, true)['skill'] as $index => $skill)
                                    @php
                                        ...
                                        $percentage = 0;
                                        match ($skillLevel) {
                                            App\Enums\SkillProfeciency::BEGINNER->value => $percentage = 33,
                                            App\Enums\SkillProfeciency::INTERMEDIATE->value => $percentage = 66,
                                            App\Enums\SkillProfeciency::PROFICIENT->value => $percentage = 90,
                                        };
                                    @endphp
                                    <div>
                                        ...
                                        <div class="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-[6px]">
                                            <div class="bg-emerald-600 h-[6px] rounded-full" style="width: {{ $percentage }}%"></div>
                                        </div>
                                    </div>
                                @endforeach
                            @endif
                            ...
                            @foreach(json_decode(auth()->user()->experience, true)['year'] as $index => $year)
                                <div class="flex mt-6">
                                    <div class="text-slate-400 font-semibold min-w-[80px] text-center">
                                        <img src="{{ asset('assets/images/company/circle-logo.png') }}" class="size-16 mx-auto mb-2 block"
