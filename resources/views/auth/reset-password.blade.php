                    ...
                    <div class="relative overflow-hidden bg-white dark:bg-slate-900 shadow-md dark:shadow-gray-800 rounded-md">
                        <div class="p-6">
                            ...
                            <div class="grid grid-cols-1">
                                ...
                                <form class="text-start" action="{{ route('password.store') }}" method="POST">
                                    @csrf
                                    <div class="grid grid-cols-1">
                                        <div class="mb-4 text-start">
                                            <label class="font-semibold" for="LoginEmail">Email Address:</label>
                                            <input type="hidden" name="token" value="{{ $request->route('token') }}">
                                            <input id="LoginEmail" type="email" name="email" value="{{old('email', $request->email)}}" class="form-input mt-3 rounded-md" placeholder="name@example.com">
                                            @error('email')
